...
Earthworm Jim 2 (Europe) - \[PAL to NTSC v1.3 by Sakitoshi\]\
*This hack is included because Earthworm Jim 2 is a PAL exclusive game and the hack allows it to be played in NTSC at 60hz instead of the slowed down PAL 50hz.*
...
Xenogears Perfect Works Build contains many different patches that can be mixed and matched. Most of the project is mature enough to be included in HackSec. However, one of the patches is a script edit/rewrite that is still in progress.

I handled this by deciding on a few different combinations of the non-script patches to include in HackSet, but the script patch isn't going to be included until it is complete.